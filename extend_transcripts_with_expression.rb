...
expression_by_enst = Hash[ File.readlines('transcripts_expression.txt').map(&:strip).map(&:split) ]
...
File.open('source_data_2/reads_vs_hg19_gencodeComprehensive.stats') do |f|
  f.each_line do |line|
