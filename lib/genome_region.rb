...
module UpDownStream
  ...
  def expand_upstream_with_peaks(peaks)
    ...
    if peaks_intersecting_region.empty?
      ...
      most_upstream_peak_region = peaks_intersecting_region.map(&:region).min
      region_expansion = self.most_upstream_region.upstream(Float::INFINITY).intersection(most_upstream_peak_region.with_downstream(Float::INFINITY))
      self.union(region_expansion)
    end
  end
end
