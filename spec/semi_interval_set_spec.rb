...
describe 'Regions and region sets' do
  ...
  describe '#region_adjacent?' do
    ...
    { [region_set(:left, :central, :right), interval(:central)] => UnsupportedType,
      [region_set(:left, :central, :right), empty_interval] => UnsupportedType,
      ...
      specify("#{first_region} region_adjacent? #{second_region} should raise ImpossibleComparison") {
        expect{ first_region.region_adjacent?(second_region) }.to raise_error(exception_type)
      }
      specify("#{second_region} region_adjacent? #{first_region} should raise ImpossibleComparison") {
